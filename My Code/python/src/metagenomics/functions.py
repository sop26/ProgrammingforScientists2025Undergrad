def richness_map(all_maps: dict[str, dict[str, int]]) -> dict[str, int]:
    ...
    """
    richness_map = {}
    for key in all_maps.keys():
        richness_map[key] = richness(all_maps[key])
...
def simpsons_map(all_maps: dict[str, dict[str, int]]) -> dict[str, float]:
    ...
    simpsons_map = {}
    for key in all_maps.keys():
        simpsons_map[key] = simpsons_index(all_maps[key])
    ...
    for i in range (0, len(samples_sorted)):
        for j in range(i+1, len(samples_sorted)):
            if dist_metric == "Bray-Curtis":
                beta_matrix[i][j] = bray_curtis_distance(all_maps[samples_sorted[i]][samples_sorted[j]])
                beta_matrix[j][i] = bray_curtis_distance(all_maps[samples_sorted[i]][samples_sorted[j]])
            elif dist_metric == "Jaccard":
                beta_matrix[i][j] = jaccard_distance(all_maps[samples_sorted[i]][samples_sorted[j]])
                beta_matrix[j][i] = jaccard_distance(all_maps[samples_sorted[i]][samples_sorted[j]])
            else:
                raise ValueError()
        beta_matrix[i][i] = 0
